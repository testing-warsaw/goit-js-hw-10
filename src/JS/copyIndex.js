  ...
  const apiUrl = `${baseUrl}/name/${findCountry}`
    ...
    .then((response) => {
      ...
      .then((data) => {
        ...
        if(data.length >= 10){
          console.log("Za dużo wyników Alert!");
  ...
  function renderCountriesList(countries) {
    ...
    const marcup = countries.map((country) => {
      ...
      for(const language in country.languages) {
        ...
        tablicaLang.push(country.languages[language])
        
      }
      const tablicaLangString = tablicaLang.join(", ")
      console.log(tablicaLangString);
        ...
        return `<li>
            ...
            <p><b>Languages </b>: ${tablicaLangString}</p>
...
function renderCountriesInfo(countries) {
  ...
  countryList.insertAdjacentHTML("beforebegin", singleCountry)
}
